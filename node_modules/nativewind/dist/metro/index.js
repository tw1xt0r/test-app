...
function withNativeWind(metroConfig, { input, outputDir = ["node_modules", ".cache", "nativewind"].join(path_1.default.sep), projectRoot = process.cwd(), inlineRem = 14, configPath: tailwindConfigPath = "tailwind.config.js", cliCommand = "npx tailwindcss", hotServerOptions = {}, } = {}) {
    if (!input) {
        throw new Error("withNativeWind requires an input parameter: `withNativeWind({ input: <css-file> })`");
    }
    const output = path_1.default.resolve(projectRoot, path_1.default.join(outputDir, input));
    ...
    metroConfig.transformer = {
        ...
        getTransformOptions: async (entryPoints, options, getDependenciesOf) => {
            if (!output || !input)
                throw new Error("Invalid NativeWind config");
